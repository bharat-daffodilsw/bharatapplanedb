...
var globalCollections = {
        "pl.collections": {collection: Constants.Admin.COLLECTIONS, fields: [
            ...
            {field: "triggers", type: "object", multiple: true, fields: [
                {"field": "functionName", type: "string"},
                {"field": "operations", type: "string", multiple: true, options: ["insert", "update", "delete"]},
                {"field": "when", type: "string", options: ["pre", "post"]},
...
function executeQuery(query, db, collection, finalCallback) {
    if (query[Constants.Query.DATA]) {
        finalCallback(null, query[Constants.Query.DATA]);
        return;
    }
    collection.get(Constants.Admin.Collections.MERGE, function (err, merge) {
        ...
        if (query[Constants.Query.UNWIND] || query[Constants.Query.GROUP]) {
            ...
            if (merge && db.db.databaseName !== Config.Admin.DB && Object.keys(merge).length > 0) {
                ...
                log = populateInitialLog("mongoquery", {collection: collection.mongoCollection.collectionName, pipeLines: JSON.stringify(pipeLines)});
            ...
            if (merge && db.db.databaseName !== Config.Admin.DB && Object.keys(merge).length > 0) {
                ...
                log = populateInitialLog("mongoquery", {collection: collection.mongoCollection.collectionName, filter: JSON.stringify(filter), options: JSON.stringify(options)});
...
function mergePipelineData(db, collection, merge, pipeLines, finalCallback) {
    var log = undefined;
    var callback = function (err, res) {
        populateFinalLog(db, log);
        finalCallback(err, res);
...
function mergeQueryData(db, collection, merge, filter, options, callback) {
    ...
    if (mergeCollection === "override") {
        ...
        collection.count({}, options, function (err, count) {
            if (count > 0) {
                log = populateInitialLog("mongoquery", {collection: collection.mongoCollection.collectionName, filter: JSON.stringify(filter), options: JSON.stringify(options)});
                collection.find(filter, options).toArray(callback1);
            } else {
                db.adminDB(function (err, adminDb) {
                    if (err) {
                        callback(err);
                    ...
                    log = populateInitialLog("mongoquery", {collection: collection.mongoCollection.collectionName, filter: JSON.stringify(filter), options: JSON.stringify(options)});
                    adminDb.db.collection(collection.mongoCollection.collectionName).find(filter, options).toArray(callback1);
                })
            }
        });
    } else {
        db.adminDB(function (err, adminDb) {
            ...
            var log = populateInitialLog("mongoquery", {collection: collection.mongoCollection.collectionName, filter: JSON.stringify(filter), options: JSON.stringify(options)});
            adminDb.db.collection(collection.mongoCollection.collectionName).find(filter, options).toArray(function (err, adminResult) {
                ...
                log = populateInitialLog("mongoquery", {collection: collection.mongoCollection.collectionName, filter:JSON.stringify({_id: {$in: adminIds}}) , options: JSON.stringify(options)});
...
DB.prototype.batchUpdateById = function (batchUpdates, finalCallback) {
    try {
        if (!finalCallback || (typeof finalCallback != 'function')) {
            throw new Error("callback not defined in DB.prototype.batchUpdate");
        }
        var log = populateInitialLog("update", JSON.stringify(batchUpdates));
        var callback = function (err, res) {
            populateFinalLog(that, log);
            finalCallback(err, res);
        }
        var that = this;
        var batchResult = {};
...
DB.prototype.query = function (query, finalCallback) {
    try {
        var queryId = Utility.getUnique();
        var that = this;
        if (!query[Constants.Query.COLLECTION]) {
            finalCallback(new ApplaneDBError(Constants.ErrorCode.MANDATORY_FIELDS.MESSAGE + "[" + Constants.Query.COLLECTION + "]", Constants.ErrorCode.MANDATORY_FIELDS.CODE))
            return;
        }
        var log = populateInitialLog("query", JSON.stringify(query));
        var callback = function (err, res) {
            populateFinalLog(that, log);
            finalCallback(err, res);
        }
...
DB.prototype.invokeFunction = function (functionName, parameters, finalCallback) {
    var that = this;

//    console.log(functionName);
    var log = populateInitialLog("invoke", {function: functionName, parameters: JSON.stringify(parameters)});
    var callback = function (err, res) {
        populateFinalLog(that, log);
        finalCallback(err, res);
    }
...
function populateInitialLog(type, info) {
    var log = {};
    log["type"] = type;
...
function populateFinalLog(that, log) {
...
DB.prototype.iterateLocalDBs = function (task) {
    ...
    that.adminDB(function (err, admindb) {
            ...
            admindb.queryAsPromise({$collection: "pl.dbs"}).then(function (databaseNames) {
                Utility.iterateArrayWithPromise(databaseNames, function (index, dbName) {
                    var d1 = Q.defer();
...
DB.prototype.onStartProcessOnCommitQueue = function () {
    var d = Q.defer();
    this.iterateLocalDBs(function (db) {
...
DB.prototype.addToOnCommitQueue = function (batchUpdates) {
    ...
    this.updateAsPromise(inserts).then(function () {
        D.resolve();
    }).fail(function (e) {
        D.reject(e);
    });
...
DB.prototype.processOnRollbackQueue = function (callback) {
    ...
    this.updateAsPromise(remove).then(function () {
        D.resolve();
    }).fail(function (e) {
        D.reject(e);
    });
    ...
    setImmediate(function () {
        that.query({$collection: "pl.asyncqueue", $filter: {status: "commit"}}, function (err, queueData) {
            ...
            if (queueData && queueData.length > 0) {
                ...
                that.updateAsPromise(operation).then(function () {
                    var updates = [
