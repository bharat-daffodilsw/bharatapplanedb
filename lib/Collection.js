...
Collection.prototype.insertAsPromise = function (inserts, modules, options) {
    ...
    Utils.populate_IdInArray(inserts);
    ModuleManager.triggerModules("onInsert", document, that, that.db, {}).then(
        function () {
            console.log("Calling onSave")
            return ModuleManager.triggerModules("onSave", document, that, that.db, {pre:true});
        }).then(
        function () {
            var d1 = Q.defer();
            Q.delay(0).then(
...
//                    return d1.promoise;
        ...
        function (insertedValue) {
            console.log("firing on save events")
            document.updates._id = insertedValue._id;
            console.log("document in onSave after >>" + JSON.stringify(document))
            return EventManager.triggerEvents("onSave", document, events, that.db, {post:true});
        }).then(
        function () {
            console.log("..insret complete")
            d.resolve();
        }).fail(function (err) {
            d.reject(err);
        })
