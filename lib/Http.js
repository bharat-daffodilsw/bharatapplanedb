    ...
    app.all("/rest/disconnect", function (req, res) {
        runInDomain(req, res, function () {
            ...
            disconnect(token, function (err) {
                writeJSONResponse(req, res, err || {logout: true});
            })
        })
            ...
            getConnection(token, function (err, db) {
                ...
                db.startTransaction(function (err, txid) {
                    ...
                    db.batchUpdateById(batchUpdate, function (err, result) {
                        if (err) {
                            db.rollbackTransaction(function (error) {
                                writeJSONResponse(req, res, err || result);
                            });
                            return;
                        }
                        db.commitTransaction(function (error) {
                            writeJSONResponse(req, res, error || result);
                        });
                    })
                });
