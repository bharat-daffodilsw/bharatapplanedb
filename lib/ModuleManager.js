...
//var modules = [
//    {index:1, name:"NumberModule", path:"./modules/NumberModule.js", require:{update:["TriggerModule"]}, events:[
...
//TODO exclude modules should be part of options


function fireEvent(event, doc, fields, updatedField, collection, module, db, options) {
    console.log("MMM fire event>>>" + JSON.stringify(doc))
    console.log("MMM fire event updatedField>>>" + JSON.stringify(updatedField))
    console.log("MMM fire event module>>>" + JSON.stringify(module))
    var fieldInfo = getFieldDef(updatedField, fields, options);
    console.log("MMM field info>>>" + JSON.stringify(fieldInfo))
    var e = getEvent(event, module, fields, updatedField, fieldInfo, options)
    ...
    options = Utility.deepClone(options);
...
exports.triggerModules = function (event, doc, collection, db, options) {
    console.log("....triggerModules>>>options>>" + JSON.stringify(options));
    var D = Q.defer();
    populateModuleSequence("update");
    collection.getAsPromise("fields").then(
        function (fields) {
            console.log("Fields>>>" + JSON.stringify(fields));
            console.log("updateModules>>>>" + JSON.stringify(updateModules))
            var D0 = Q.defer();
            Utility.iterateArrayWithPromise(updateModules,
                function (index, module) {
                    console.log("Module>>>" + JSON.stringify(module));
                    var d1 = Q.defer();
                    var updatedFields = options.fields;
                    Utility.iterateArrayWithPromise(updatedFields,
                        function (updatedFieldIndex, updatedField) {
                            //this will run only for type base module like number module
                            console.log("updatedFieldIndex>>>>" + updatedFieldIndex)
                            return fireEvent(event, doc, fields, updatedField, collection, module, db, options);
                        }).then(
                        function () {
                            //this will run for non type based module like trigger module
                            console.log("fire event after all updated fields")
                            return fireEvent(event, doc, fields, undefined, collection, module, db, options);
                        }).then(
                        function () {
                            console.log("resolve promise of module")
                            d1.resolve();
                        }).fail(function (err) {
                            d1.reject(err);
                        });
        ...
        function () {
            console.log("resolve promise of module manager")
            D.resolve();
        }).fail(function (err) {
            D.reject(err);
        });
...
function getEvent(event, module, fields, field, fieldInfo, options) {
...
function getFieldDef(field, fields, options) {
